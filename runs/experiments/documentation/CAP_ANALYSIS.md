...
At **penalty=150, min=700**, only **cap22** was run (best Top5 magnets in that batch vs `penalty100_min700_cap21`).

**Default for new runs:** cap **2/2** (matches current winner `penalty300_min700_cap22_queryv2`). Skip cap-only sweeps unless a new (penalty, min) pair looks cap-sensitive.
