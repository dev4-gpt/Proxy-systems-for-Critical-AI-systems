...
2b. **Disentanglement re-run at the ORIGINAL n=30 allocation — DONE (2026-06-19).** Because the n=25 run confounded authentication with a rubric/n change, the pipeline was re-run authenticated at the *original* n=30 (10/10/10) allocation, forced via `--rubric configs/projected_pair_rubric_old.json --out-dir <temp>` (committed rubric untouched), and copied to **`projected_pairs/full_summary_authenticated_n30.json`**: n=30, **Spearman ρ=+0.6913 (p=2.34e-5)**, **Pearson r=+0.6643 (p=6.25e-5)**, paired-t 2.574 (p=0.015), **`go`=true**, `authenticated=true` (49×200 / 5×403 = 9.3%). **Holding n fixed at 30 and flipping only authentication moves ρ from −0.2107 → +0.6913 (fails→passes), so authentication — removing the 38% 403 starvation in the frozen run — is the real driver of the flip, not the rubric/n change.** See [§9](#9-component-6--statistics-discrimination--cross-method-spearman).
...
- **Honest weaknesses are documented, not hidden.** Cross-method Spearman ρ = **−0.21** (p = 0.26) on the frozen unauthenticated n=30 run **fails** the project's own 0.30 rubric. **Caveat (2026-06-19) — now disentangled:** authenticated re-runs flip this to a passing positive at **both** allocations — ρ = **+0.78** (`go`=true) under the current rubric (n=25, `full_summary_authenticated.json`) **and** ρ = **+0.69** (`go`=true) at the *original* n=30 allocation (`full_summary_authenticated_n30.json`). Because the n=30 run holds the allocation fixed and changes only authentication, it shows **authentication (removing the 38% 403 starvation) — not the rubric/n change — drove the flip**; the original negative ρ is largely an artifact of unauthenticated rate-limiting. This is **analysis flagged for the author's decision** and has **not** been propagated into the paper docs (see [§0.1](#01--2026-06-19-authenticated-re-run-pass-network-outside-sandbox) and [§9](#9-component-6--statistics-discrimination--cross-method-spearman)).
...
**Authenticated re-runs (2026-06-19) — captured separately, decision needs author sign-off.** The deferred authenticated regeneration was executed on the real network and saved to **new** files so the frozen reference above is not destroyed. Two authenticated runs were captured: one under the *current* committed rubric (n=25), and one forced back to the *original* n=30 (10/10/10) allocation to disentangle authentication from the rubric/n change:
...
**⚠ This is analysis to inform a decision — paper claims are intentionally NOT yet changed.** `PAPER_PACKAGE.md`, `VALIDATION_MEMO.md`, `WORK_REVIEW.md`, etc. still cite the frozen ρ=−0.21 and are **left unchanged pending the author's call**. Given the disentanglement above, the recommended decision is to **adopt an authenticated run as canonical** (the n=30 `full_summary_authenticated_n30.json` is the apples-to-apples comparison to the frozen baseline; the n=25 `full_summary_authenticated.json` is the current-rubric result) and rewrite the cross-method-agreement narrative from "methods don't agree (ρ negative)" to "methods agree once rate-limiting is removed (ρ≈+0.69–0.78, `go`=true)". Keep the neutral *labeled* framing and retain the frozen unauthenticated artifact as a documented rate-limiting caveat. The labeled cohort was already authenticated (see §0).
